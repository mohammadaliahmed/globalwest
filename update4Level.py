...
try:
    data = pd.read_excel(r'4level.xlsx')
    fileNameToWrite="buick4.txt"
    df = pd.DataFrame(data, columns=['main', 'child', 'subchild','categoryUrl'])
            ...
            for text2 in special_divs2:
                ...
                if len(download) > 0:
                    for text in download:
                        hrefText = (text['href'])
                        fourtLevel = text.text
                        for text in special_divs2:
                            download = text.find_all('h1')
                            secondLevelCategoryName = download[0].text
                            print("child:  " + secondLevelCategoryName + "    " + secondLevelCategoryName + " " + hrefText)

                            data = {
                                "name": fourtLevel,
                                "parent": str(thirdLevelCategoryId)
                            }
                            abcaaa = wcapi.post("products/categories", data).json()

                            try:
                                fourthLevelId = abcaaa["id"]
                            except:
                                fourthLevelId = abcaaa["data"]["resource_id"]
